...
class MIMIC4Extractor(MIMIC4Paths):
    ...
    def extract_patient_information(self) -> pl.LazyFrame:
        ...
        return (
            ...
            .with_columns(
                ...
                pl.col("dod").str.to_datetime("%Y-%m-%d"),  # hour and minute are not provided
    ...
    def _extract_patient_height_weight(self, icustays: pl.LazyFrame, force=False) -> pl.DataFrame:
        ...
        # ITEMIDS = {
        #     "weight_kg": 762,  # Admit Wt [carevue]
        #     "weight_kg": 763,  # Daily Weight [carevue]
        #     "weight_kg": 3580,  # Present Weight  (kg) [carevue]
        #     "weight_kg": 3693,  # Weight Kg [carevue]
        #     "weight_kg": 224639,  # Daily Weight [metavision]
        #     "weight_kg": 226512,  # Admission Weight (Kg) [metavision]
        #     "weight_lbs": 3581,  # Present Weight  (lb) [carevue]
        #     "weight_lbs": 226531,  # Admission Weight (lbs.) [metavision]
        #     "height_inch": 920,  # Admit Ht [carevue]
        #     "height_inch": 1394,  # Height Inches [carevue]
        #     "height_inch": 226707,  # Height [metavision]
        #     "height_cm": 226730,  # Height (cm) [metavision]
        # }
        ...
        #     .select(self.icu_stay_id_col, "itemid", "valuenum", "charttime")
        #     .join(icustays.select(self.icu_stay_id_col, "intime"), on=self.icu_stay_id_col)
        #     .with_columns(
        #         pl.col("charttime").str.to_datetime("%Y-%m-%d %H:%M:%S"),
        #         pl.col("intime").str.to_datetime("%Y-%m-%d %H:%M:%S"),
        #         pl.col(self.icu_stay_id_col).cast(int, strict=False),
        #     )
        #     .with_columns((pl.col("charttime") - pl.col("intime")).alias("measuretime"))
        #     # Filter for weight and height measurements within ADMISSION_WEIGHT_HEIGHT_CUTOFF
        #     .filter(
        #         pl.col("itemid").is_in(KEEPIDS),
        #         pl.col("measuretime").le(pl.duration(hours=self.ADMISSION_WEIGHT_HEIGHT_CUTOFF)),
        #     )
        #     .drop("measuretime", "intime")
        #     .cast({"itemid": pl.String})
        #     .group_by([self.icu_stay_id_col, "charttime", "itemid"])
        #     .first()
        #     .collect(streaming=True)
        #     # Pivot the table to have one row per STAY_ID and CHARTTIME
        #     .pivot(index=[self.icu_stay_id_col, "charttime"], columns="itemid", values="valuenum")
        ...
        #     )
        #     # Approximate admission weight / height as the mean of all weight measurements
        #     # within ADMISSION_WEIGHT_HEIGHT_CUTOFF
        #     .with_columns(
        #         pl.concat_list(pl.col("height_cm", "height_inch_in_cm"))
        #         .list.mean()
        #         .alias(self.height_col),
        #         pl.concat_list(pl.col("weight_kg", "weight_lbs_in_kg"))
        #         .list.mean()
        #         .alias(self.weight_col),
        #     )
        #     .drop(*ITEMIDS.keys(), "height_inch_in_cm", "weight_lbs_in_kg")
        #     # Keep only the last measurement per ICU stay
        #     .sort("charttime")
        #     .group_by(self.icu_stay_id_col)
        #     .last()
        # )
        ...
        height_weight.write_parquet(self.precalc_path + "MIMIC4_height_weight.parquet")
    ...
    def extract_medications(self) -> pl.LazyFrame:
        ...
        mimic4_medication_mapping = self.helpers.load_many_to_many_to_one_mapping(
    ...
    def extract_diagnoses(self) -> pl.LazyFrame:
        ...
        d_diagnoses = pl.scan_csv(self.d_icd_diagnoses_path, dtypes={"icd_code": str})

        return (
            diagnoses.select(
                self.person_id_col, self.hospital_stay_id_col, "icd_code", "icd_version", "seq_num"
            )
    ...
    def extract_procedures(self) -> pl.LazyFrame:
        ...
        procedures_icd = pl.scan_csv(self.procedures_icd_path, dtypes={"icd_code": str}).rename(
            {
                "subject_id": self.person_id_col,
                "hadm_id": self.hospital_stay_id_col,
            }
        )
        d_icd_procedures = pl.scan_csv(self.d_icd_procedures_path, dtypes={"icd_code": str})

        procedureevents = (
            procedureevents.select([self.icu_stay_id_col, "starttime", "endtime", "itemid"])
            .join(intimes, on=self.icu_stay_id_col)
        ...
        procedures_icd = (
            ...
            .join(
                d_icd_procedures.select("icd_code", "icd_version", "long_title"),
                on="icd_code",
            )
            .rename(
                {
        ...
        return pl.concat([procedureevents, procedures_icd], how="diagonal_relaxed")
